  ...
  return (
    <div className="flex flex-col gap-5 px-5 pb-8 pt-10">
        ...
        <div className="flex gap-2">
          {themes.map(({ value, label, icon: Icon }) => (
            <button key={value} onClick={() => setTheme(value)} className={cn(
              ...
              <span className={cn("text-xs font-semibold", theme === value ? "text-gold-500" : "text-muted")}>{label}</span>
            </button>
          ))}
        </div>
      </Section>
      ...
      <Section icon={Volume2} title="Sound & Haptics">
        ...
        >
          {soundOn ? <Volume2 size={18} className="text-gold-500" /> : <VolumeX size={18} className="text-muted" />}
          <div className="flex-1 text-left">
            <p className="text-sm font-semibold">{soundOn ? "Sounds On" : "Sounds Off"}</p>
            <p className="text-muted text-[12px]">Tap feedback, chimes & haptics</p>
          </div>
      ...
      <Section icon={Type} title="Text Size">
        <div className="flex items-center gap-3">
          <button onClick={() => setFontSize(fontSize - 1)} disabled={fontSize <= 12} className="flex h-8 w-8 items-center justify-center rounded-full bg-surface active:scale-90 disabled:opacity-30">
            <Minus size={14} />
          </button>
          ...
          <button onClick={() => setFontSize(fontSize + 1)} disabled={fontSize >= 24} className="flex h-8 w-8 items-center justify-center rounded-full bg-surface active:scale-90 disabled:opacity-30">
            <Plus size={14} />
          </button>
        ...
        {/* Typeface selector */}
        <p className="text-muted mt-1 text-2xs font-bold uppercase tracking-caps">Typeface</p>
        <div className="flex gap-2">
        ...
        <div className="mt-3 rounded-xl bg-surface p-3">
          <p className="text-muted mb-1 text-2xs font-semibold uppercase tracking-caps">Preview</p>
          <p className="text-secondary leading-relaxed" style={readingStyle}>
            "For God so loved the world, that he gave his only begotten Son, that whosoever believeth in him should not perish, but have everlasting life."
          </p>
          <p className="text-muted mt-1 text-[11px] italic" style={{ fontFamily: FONT_CSS[fontFamily] }}>
      ...
      <Section icon={BookOpen} title="Bible Version">
        <div className="flex flex-wrap gap-2">
          {bibleVersions.map((v) => (
            <button key={v} onClick={() => setBibleVersion(v)} className={cn(
              "rounded-lg px-4 py-2 text-sm font-semibold transition-all",
              bibleVersion === v ? "bg-gold-500 text-navy-900 shadow-gold-glow" : "bg-surface text-muted",
            )}>
              {v}
            </button>
          ))}
      ...
      <Section icon={Calendar} title="Study Plan">
        {studyPlan.configured ? (
          ...
          <p className="text-muted text-[13px]">No plan set — launch the app home screen to set up.</p>
        )}
      </Section>
      ...
      <Section icon={Bell} title="Daily Reminder">
        <button
          ...
          className={cn("flex items-center gap-3 rounded-xl p-3 transition-all", studyPlan.reminderEnabled ? "bg-gold-500/10 ring-1 ring-gold-500/20" : "bg-surface")}
        >
          {studyPlan.reminderEnabled ? <Bell size={18} className="text-gold-500" /> : <BellOff size={18} className="text-muted" />}
          <div className="flex-1 text-left">
            <p className="text-sm font-semibold">{studyPlan.reminderEnabled ? "Reminder On" : "Reminder Off"}</p>
          </div>
        ...
        {studyPlan.reminderEnabled && (
          <div className="flex items-center gap-3 animate-slide-up">
            ...
            <input
              ...
              className="flex-1 rounded-xl border bg-surface border-theme px-4 py-2.5 text-center font-bold outline-none focus:border-gold-500/40"
            />
          </div>
        )}
      </Section>
      ...
      <div className="flex items-center gap-2 rounded-xl bg-surface px-3 py-2.5">
        {isOnline ? (
          <><Wifi size={14} className="text-emerald-500" /><p className="text-muted text-[12px]">Online</p></>
        ) : (
          <><WifiOff size={14} className="text-amber-500" /><p className="text-muted text-[12px]">Offline — cached data available</p></>
        )}
      </div>
    ...
    <Section icon={Sparkles} title="AI Assistant">
        ...
        {PROVIDERS.map((p) => (
          <button
            ...
            className={cn(
              ...
              activeProvider === p.id && "ring-2 ring-gold-500/40 bg-gold-500/5",
            )}
          >
      ...
      {activeProvider && (
        <div className="flex flex-col gap-2">
          <div className="flex items-center justify-between">
            <span className="text-[13px] text-secondary font-semibold">
              {currentProvider?.emoji} {currentProvider?.name}
            </span>
            {hasKey && !editingKey && (
              <span className="text-emerald-500 text-[11px] font-semibold">Connected ✓</span>
            )}
          </div>
          ...
          {(editingKey || !hasKey) && (
            <div className="flex flex-col gap-2">
              ...
              <button onClick={handleSaveKey} className="btn-primary text-sm" disabled={!keyInput.trim()}>
                Save Key
              </button>
            </div>
          )}
          ...
          {hasKey && !editingKey && (
            <button onClick={() => { setKeyInput(getStoredKey(activeProvider) ?? ""); setEditingKey(true); }} className="btn-secondary text-xs">
              Change Key
            </button>
          )}
        </div>
